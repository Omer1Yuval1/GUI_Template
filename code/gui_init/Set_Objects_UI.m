function Set_Objects_UI(P)
	...
	for m=1:length(P.GUI_Handles.Menu_Names)
		P.GUI_Handles.Menus(m) = uimenu(P.GUI_Handles.Main_Figure,'Text',P.GUI_Handles.Menu_Names{m},'UserData',0);
	end
	Set_Menu_2(P);
	Set_Menu_3(P);
	...
	for i=1:4 % For each row.
		...
		P.GUI_Handles.Control_Panel_Objects(5-i,2) = uiradiobutton(P.GUI_Handles.Radio_Group_1,'Text',['Option ',num2str(5-i)],'UserData',[5-i,2],'FontSize',P.GUI_Handles.Buttons_FontSize,'FontColor','w');
		...
		switch(i)
		case 1
			P.GUI_Handles.Control_Panel_Objects(i,4) = uislider(Control_Panel_Grid,'UserData',[i,4],'MajorTickLabels',{});
	...
	set(P.GUI_Handles.Control_Panel_Objects(2,1),'Text','Option 2');
	set(P.GUI_Handles.Control_Panel_Objects(3,1),'Text','Option 3');
	...
	for t=1:length(Info_Tabs)
		...
		drawnow;
        Info_Grids(t) = uigridlayout(Info_Tabs(t),[1,1],'Padding',[0,0,0,0],'BackgroundColor',P.GUI_Handles.BG_Color_1);
        drawnow;
		W = Info_Tabs(t).InnerPosition(3); % {W.*0.35,W.*0.35,'auto'}
		P.GUI_Handles.Info_Tables(t) = uitable(Info_Grids(t),'Data',cell(10,3),'UserData',t,'ColumnWidth',{W.*0.35,W.*0.35,W.*0.3},'ColumnEditable',[false,true,true],'RowName',[],'ColumnName',[],'ForegroundColor','w','BackgroundColor',P.GUI_Handles.BG_Color_1); % {'1x','1x','0.2x'}
